...
export const getMostSimilarEmbeddedFiles = async (
  ...
  try {
    ...
    const mostSimilarEmbeddedFiles = await db
      ...
      .where(and(eq(embeddedFilesTable.embeddedBranchId, embeddedBranch.id)))
      .orderBy(t => desc(t.similarity))

    const filteredFiles = mostSimilarEmbeddedFiles.reduce(
      (
        acc: { files: typeof mostSimilarEmbeddedFiles; tokens: number },
        file
      ) => {
    ...
    console.error("Error in getMostSimilarEmbeddedFiles:", error)
    throw error
  }
}
